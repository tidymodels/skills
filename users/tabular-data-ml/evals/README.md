...
The evaluation set includes 5 test cases covering key machine learning workflows and best practices:
...
- Natural language with casual phrasing ("hey so i have...", "ok I split my data...")
...
- **Best practices**: Train/test splitting, cross-validation strategy, test set protection, seed setting

- **Feature engineering**: Missing data, skewed predictors, high-cardinality categoricals, normalization
...
- Appropriate evaluation metrics (classification: ROC-AUC, Brier; regression: RMSE, R²)
...
2. **Grade outputs**: Check workflow correctness, tidymodels best practices, completeness
...
- [ ] Appropriate resampling strategy (vfold_cv for standard data, sliding_period for time series)
...
- [ ] Appropriate feature engineering for data types (normalization, dummy encoding, etc.)

- [ ] High-cardinality categoricals handled (embedding, filtering, or appropriate strategy)
...
- [ ] Appropriate metrics (classification: ROC-AUC, accuracy, Brier; regression: RMSE, R², MAE)

- [ ] Visualizations (ROC curves, confusion matrices, observed vs predicted, residuals)
...
- [ ] Time series: Uses rolling origin validation, explains temporal structure importance

- [ ] Parallel processing: Sets up future package when user has multiple cores

- [ ] Test set protection: Refuses premature evaluation, explains why test set matters
...
- [ ] Explains key concepts (why cross-validation, why test set protection, etc.)
...
- Prompts intentionally vary in formality and completeness to reflect real user interactions

- Eval 3 specifically tests the skill's ability to refuse inappropriate actions

- Seed setting is a critical requirement across all applicable tests to ensure reproducibility
