...
**Never use random cross-validation for time series data.** Random CV shuffles
observations, which:
...
- **Produces optimistically biased estimates** - Performance looks better than
  it actually is
...
- **Simulates realistic forecasting** - Tests how model would perform in
  production

- **Expands training window** - Mirrors how models are used (retraining as new
  data arrives)
...
Time series cross-validation creates multiple train/test splits that respect
temporal ordering:
...
This mimics real-world forecasting where you periodically retrain and forecast
ahead.
...
> "How far ahead do you need to forecast? This will help me set up the
> resampling strategy."
...
**Suggest using the same `assess` size as the test set** to ensure consistent
evaluation.
...
- **NEVER** engineer features, then evaluate directly on training data without
  resampling
...
- **DO** ask the user how much data to use to assess the model and suggest the
...
**Key principle:** Make ALL modeling decisions using resampling. Use the test
set only once for final validation.
...
A: Each test set use reduces its validity. Resampling gives you multiple
out-of-sample evaluations without touching the test set.

**Q: How many slices should I use?**

A: 4-6 is typical. More slices give better estimates but take longer. Balance
precision with computation time.

**Q: Should `assess` match my forecast horizon?**

A: Yes, ideally. If you need 6-month forecasts in production, use
`assess = "6 months"` to evaluate that horizon.

**Q: Can I use random CV if I have external time-independent predictors?**

A: No. Even with external predictors, the temporal structure of the target
variable must be respected. Always use time series CV.
...
5. **Requires thoughtful parameter choices** - Based on forecast horizon and
   data size

Use `time_series_cv()` for all model comparisons, and reserve the test set for
final validation with user permission.
